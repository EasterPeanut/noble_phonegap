...
$q = mysqli_query($con, "SELECT fbid FROM user WHERE userid='$id'");
if( $q && $q->num_rows ){
...
$query = "UPDATE user SET location_lat=$lat, location_lon=$lon, date='".$date."', location_name='$location_name', fbid='$fbid'
WHERE userid='$id'";
echo $query;
...
VALUES ($id, $lon, $lat, $date, '$location_name')";
	 mysqli_query($con,"INSERT INTO user (userid, location_lon, location_lat, date, location_name,fbid)
VALUES ('$id', $lon, $lat,'$date', '$location_name', $fbid)");
	 echo "insert";
}
